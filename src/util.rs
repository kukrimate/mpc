...
impl<T> core::borrow::Borrow<T> for Own<T> {
  fn borrow(&self) -> &T {
    unsafe { &*self.0 }
  }
}

impl<T> core::borrow::BorrowMut<T> for Own<T> {
  fn borrow_mut(&mut self) -> &mut T {
    unsafe { &mut *self.0 }
  }
}

impl<T> core::ops::Deref for Own<T> {
  type Target = T;
...
impl<T> core::ops::DerefMut for Own<T> {
...
impl<T: std::hash::Hash> core::hash::Hash for Own<T> {
  fn hash<H: core::hash::Hasher>(&self, h: &mut H) {
    unsafe { (*self.0).hash(h); }
  }
}

impl<T: std::fmt::Display> std::fmt::Display for Own<T> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
    unsafe { (*self.0).fmt(f) }
  }
}

impl<T: std::fmt::Debug> std::fmt::Debug for Own<T> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
    unsafe { (*self.0).fmt(f) }
  }
}
...
impl<T> core::borrow::Borrow<T> for Ptr<T> {
  fn borrow(&self) -> &T {
    unsafe { &*self.0 }
  }
}
...
impl<T> core::borrow::Borrow<T> for PtrMut<T> {
  fn borrow(&self) -> &T {
    unsafe { &*self.0 }
  }
}

impl<T> core::borrow::BorrowMut<T> for PtrMut<T> {
  fn borrow_mut(&mut self) -> &mut T {
    unsafe { &mut *self.0 }
  }
}

impl<T> core::ops::Deref for PtrMut<T> {
  type Target = T;
...
impl<T> core::ops::DerefMut for PtrMut<T> {
  fn deref_mut(&mut self) -> &mut T {
    unsafe { &mut *self.0 }
  }
}
...
impl<T: std::fmt::Display> std::fmt::Display for PtrMut<T> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
    unsafe { (*self.0).fmt(f) }
  }
}

impl<T: std::fmt::Debug> std::fmt::Debug for PtrMut<T> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
    unsafe { (*self.0).fmt(f) }
  }
}
